...
Inner classes require a reference to an instance of the outer class to be created.
...
class Outer {
    ...
    class Nested {
        fun bar() {
            this@Outer.foo() // ERROR: this is not an inner class, so it has no Outer instance!
        }
    }
}
----
